...
class AnchorManager {
    ...
    fun updateTrackingStability(cameraTrackingState: TrackingState, currentPuzzleState: PuzzleState) {
        ...
        {
            if (framesSinceTrackingStable < GameConstants.MIN_TRACKING_FRAMES_FOR_ANCHOR_PLACEMENT + 10)
            { 
            ...
            if (trackable is Plane && trackable.isPoseInPolygon(hit.hitPose) && trackable.trackingState == TrackingState.TRACKING) {
                ...
                val offsetPose = hit.hitPose
                    .compose(Pose.makeTranslation(
                        worldForward[0] * GameConstants.ANCHOR_PLACEMENT_OFFSET_METERS,
                        GameConstants.ANCHOR_PLACEMENT_HEIGHT_ABOVE_PLANE,
                        worldForward[2] * GameConstants.ANCHOR_PLACEMENT_OFFSET_METERS
                    ))
