...
class LevelGenerator(private val assetLoader: AssetLoader) {
    ...
    fun generateLevelLayout(anchor: Anchor, gameState: GameState) {
        ...
        val numPlanetsToSpawn = min(
            GameConstants.INITIAL_PLANET_COUNT + (level -1) / GameConstants.LEVELS_PER_NEW_PLANET + (level / 4),
            GameConstants.MAX_PLANETS_CAP
        )

        val clusterMaxRadiusPlanets = GameConstants.CLUSTER_MAX_RADIUS_PLANETS + (level * 0.12f).coerceAtMost(3.0f)
        val planetTargetRadiusMin = GameConstants.PLANET_TARGET_RADIUS_MIN + (level * 0.01f).coerceAtMost(0.25f)
        val planetTargetRadiusMax = GameConstants.PLANET_TARGET_RADIUS_MAX + (level * 0.03f).coerceAtMost(1.2f)
        val planetMassScale = GameConstants.PLANET_MASS_SCALE_FACTOR + (level * 400f)
        val appleClusterRadius = GameConstants.CLUSTER_MAX_RADIUS_APPLE + (level * 0.03f).coerceAtMost(2.0f)
        ...
        for (i in 0..100) { // placement attempts
            ...
            val phi = Random.nextFloat() * PI.toFloat() * GameConstants.CLUSTER_VERTICAL_SPREAD_FACTOR - (PI.toFloat() * GameConstants.CLUSTER_VERTICAL_SPREAD_FACTOR / 2f)
        ...
        val minBlockAngleCos = cos(18.0 * PI / 180.0).toFloat() // Approx 18 deg cone
        ...
        for (pIdx in 0 until numPlanetsToSpawn) {
            ...
            for (attempt in 0..300) { // Max placement attempts per planet
                val r = GameConstants.CLUSTER_MIN_DIST_PLANETS_FROM_ANCHOR + Random.nextFloat() * (clusterMaxRadiusPlanets - GameConstants.CLUSTER_MIN_DIST_PLANETS_FROM_ANCHOR)
                val theta = Random.nextFloat() * 2f * PI.toFloat()
            ...
            if (planetLocalAttemptPos != null) {
                ...
                val textureIdx = planetsSuccessfullyPlaced % max(1, assetLoader.planetTextures.size)
        ...
        if (level >= GameConstants.MOON_START_LEVEL) {
            ...
            if (moonShouldSpawn) {
                ...
                for (mIdx in 0 until moonCount) {
                    ...
                    val orbitRadius = Random.nextFloat() * (GameConstants.MOON_ORBIT_RADIUS_MAX - GameConstants.MOON_ORBIT_RADIUS_MIN) + GameConstants.MOON_ORBIT_RADIUS_MIN
                    val orbitSpeed = Random.nextFloat() * (GameConstants.MOON_ORBIT_SPEED_MAX - GameConstants.MOON_ORBIT_SPEED_MIN) + GameConstants.MOON_ORBIT_SPEED_MIN
                    ...
                    val textureIdx = mIdx % max(1, assetLoader.moonTextures.size)
